...
import { camelize } from "../global/utils/string-conversion.util";
import { en, fr } from "../global/supportedLanguages.json";
...
export function getPreselectedSubCategories(generalCategory) {
  ...
  return _convertToObjArray(loadAllTranslation(lang), (key) => {
    const camelCaseGeneralCat = camelize(
      generalCategory?.replace("generalMenu", "")
    );
...
function _convertToObjArray(obj, filterFunction) {
  const t = Object.values(
    Object.keys(obj)
      ...
      .map((object) => {
        return { key: object, value: obj[object] };
      })
  );
  return t;
}
